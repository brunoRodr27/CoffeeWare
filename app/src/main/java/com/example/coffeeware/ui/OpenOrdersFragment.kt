...
import com.example.coffeeware.databinding.FragmentOpenOrdersBinding
...
class OpenOrdersFragment : Fragment() {
                ...
                override fun onDataChange(snapshot: DataSnapshot) {
                    if (snapshot.exists()) {
                        ...
                        for (snap in snapshot.children) {
                            ...
                            if (order.status == 0)  orderList.add(order)
                        }
    ...
    private fun initClicks() {
        binding?.fabAdd?.setOnClickListener{
            findNavController().navigate(R.id.action_homeFragment_to_addOrdersFragment)
        }
    }
